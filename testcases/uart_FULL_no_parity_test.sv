class uart_FULL_no_parity_test extends uart_base_test;
	`uvm_component_utils(uart_FULL_no_parity_test)
	...
	function new(string name = "uart_FULL_no_parity_test", uvm_component parent);
			...
			foreach(stp_bit[i_stp]) begin
				foreach(bd_rate[i_bd]) begin
					foreach(prt_error[i_error]) begin
						...
						if (cfg_lhs.randomize() with {parity_mode == NONE; 
...
endclass: uart_FULL_no_parity_test
